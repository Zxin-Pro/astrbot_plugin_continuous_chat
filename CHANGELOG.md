...
- **移除 LLM 拦截**：不再拦截任何 LLM 请求。未唤醒用户保持 AstrBot 原生行为；
  插件只负责一件事：唤醒后的用户每句话都触发 AI 回复
- 平台侧唤醒（@、引用回复机器人、唤醒前缀）同样会激活连续对话状态

## [v1.0.1] - 2026-09-16

### 修复

- 修复引用回复机器人消息、唤醒前缀触发 LLM 时被本插件误拦截的问题：平台侧唤醒
  （@、引用回复、唤醒前缀）现在同样会激活用户的连续对话状态
